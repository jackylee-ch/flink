...
/**
 * PR-A10 / S1-9 regression test for the DELETE batch dispatcher. Mirrors the PUT-path test —
 * when {@code VectorizedExecutor.invokeVectorizedBatchDelete} throws, every per-row {@link
 * StateRequest}'s future must be completed exceptionally with the thrown cause.
 */
class BatchedDeleteFailurePropagationTest {

    @Test
    void batchedDeleteFailure_propagatesToEveryPerRowFuture() {
        RuntimeException cause = new RuntimeException("simulated FFI failure on DELETE batch");
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                futures.add(fut);
                // CLEAR routes through recordDelete.
