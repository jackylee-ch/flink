...
/**
 * PR-A10 / S1-9 regression test for the GET batch dispatcher. Mirrors the PUT/DELETE-path
 * tests — when {@code VectorizedExecutor.invokeVectorizedBatchGet} throws, every per-row
 * {@link StateRequest}'s future must be completed exceptionally with the thrown cause. The
 * GET path additionally exercises the new {@code completeGetExceptionally} helper since the
 * success path materializes a deserialized value.
 */
class BatchedGetFailurePropagationTest {

    @Test
    void batchedGetFailure_propagatesToEveryPerRowFuture() {
        RuntimeException cause = new RuntimeException("simulated FFI failure on GET batch");
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                futures.add(fut);
                // VALUE_GET routes through recordGet.
