 ...
 *     1. tryFold(key, value) — cache hit: combine in-place, mark dirty, done
 ...
 *
 * @param <T> the value type (for ReducingState, accumulator == value type)
 */
@Internal
public class ForStRsReducingStateV2<T> {
    ...
    private final PendingMissTable<T, T> pendingMisses;
     ...
     */
    public T get(byte[] compositeKey) {
        return cache.peek(compositeKey);
    }
